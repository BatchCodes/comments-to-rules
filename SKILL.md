...
- The script drops comments shorter than 20 characters before you see them (`--min-length` to change this). It excludes any ` ```suggestion ` block from that length check first. This is a mechanical filter for pure noise — `lgtm`, `nit`, `+1`, and similar. It does not replace the judgment-based filtering in Step 2.
